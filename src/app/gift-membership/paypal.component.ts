...
export class PayPalSubmit {
    ...
    public handlePaypalSubmitted(resp: any, item: GiftMembership, isValid: boolean): void {
        if(!isValid) {
            return;
        }
        ...
        this.model.comments = resp.paymentToken.id + ' (' + this.model.comments + ')';
    ...
    sendAckowledgmentEmail(): string {
        ...
        const body = this.emailService.toGiftInvoiceBody(this.member.memberName,
             paidThruDate,this.model.comments, this.model.duesQuantity, this.model.duesAmount, this.model.merchandisePackageAmount, this.model.shippingCharges);
